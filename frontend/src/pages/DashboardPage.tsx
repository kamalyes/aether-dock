...
import { Zap, Server, Package, Wrench, Plus, Search, Download, Globe, Clock, ArrowRight, Trash2, CheckCircle2, XCircle, AlertCircle } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  const { skills, total: skillTotal, sources, fetchSkills, fetchSources } = useSkillStore()
  const { servers, total: mcpTotal, fetchServers } = useMcpStore()
  ...
  const loadActivities = async () => {
    const resp = await wailsApi.listActivities(10)
      ...
      <div className="max-w-5xl mx-auto px-6 py-6 space-y-6">
          ...
          <div>
            ...
            {skills.length === 0 ? (
              <div className="glass-card p-8 text-center">
                <Zap style={{ width: 32, height: 32, color: 'var(--c-text-faint)', margin: '0 auto 8px' }} />
        ...
        <FadeIn delay={0.45}>
          <div>
            ...
            {activities.length === 0 ? (
              <div className="glass-card p-8 text-center">
                <Clock style={{ width: 32, height: 32, color: 'var(--c-text-faint)', margin: '0 auto 8px' }} />
                ...
                {activities.map((activity, idx) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ borderBottom: idx < activities.length - 1 ? '1px solid var(--c-border)' : 'none' }}
                      onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--c-bg-card-hover)' }}
                      onMouseLeave={(e) => { e.currentTarget.style.background = '' }}
                    >
