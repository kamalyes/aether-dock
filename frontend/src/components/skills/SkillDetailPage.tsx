          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span
                style={{
                  fontSize: 10,
                  fontWeight: 600,
                  color: skill.status === 'installed' ? 'var(--c-green)' : 'var(--c-orange)',
                  background: skill.status === 'installed' ? 'rgba(22, 166, 106, 0.1)' : 'rgba(245, 158, 11, 0.1)',
                  padding: '2px 8px',
                  borderRadius: 999,
                }}
              >
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              {skill.tags?.map((tag) => (
                <span
                  key={tag}
                  style={{ fontSize: 10, color: 'var(--c-text-secondary)', background: 'var(--c-bg-input)', padding: '2px 8px', borderRadius: 6, border: '1px solid var(--c-border)' }}
                >
                  {tag}
                </span>
              ))}
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === 'verify' && <VerifyTab contentHash={contentHash} relatedFiles={relatedFiles} installLocations={installLocations} copiedHash={copiedHash} copyToClipboard={copyToClipboard} />}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {skill.tags.map((tag) => (
                    <span
                      key={tag}
                      style={{ fontSize: 11, color: 'var(--c-text-secondary)', background: 'var(--c-bg-input)', padding: '3px 10px', borderRadius: 999, border: '1px solid var(--c-border)' }}
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {skill.enabledTools.map((tool) => {
                    ...
                    const brandIcon = TOOL_ICONS[info.label] || TOOL_ICONS[tool]
                    return (
                      <span
                        key={tool}
      ...
      <div className="space-y-4">
        ...
        <div className="glass-card p-4">
          ...
          <div className="space-y-3">
            ...
            {skill.gitBranch && (
              <div className="flex items-center justify-between">
                <span style={{ fontSize: 12, color: 'var(--c-text-muted)' }}>Git 分支</span>
        ...
        {gitInfo && (
          <div className="glass-card p-4">
            ...
            <div className="space-y-3">
              ...
              {gitInfo.lastUpdated && (
                <div className="flex items-center justify-between">
                  <span style={{ fontSize: 12, color: 'var(--c-text-muted)' }}>最近更新</span>
        ...
        {/* Basic Info */}
        <div className="glass-card p-4">
          <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--c-text)', margin: '0 0 12px' }}>基础信息</h3>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span style={{ fontSize: 12, color: 'var(--c-text-muted)' }}>来源类型</span>
              <span style={{ fontSize: 12, color: 'var(--c-text-secondary)' }}>
                {skill.gitUrl ? 'Git 仓库' : '本地安装'}
              </span>
            </div>
            {skill.version && (
              <div className="flex items-center justify-between">
                <span style={{ fontSize: 12, color: 'var(--c-text-muted)' }}>版本</span>
                <span style={{ fontSize: 12, color: 'var(--c-text-secondary)' }}>v{skill.version}</span>
              </div>
            )}
            <div className="flex items-center justify-between">
              <span style={{ fontSize: 12, color: 'var(--c-text-muted)' }}>安装日期</span>
              <span style={{ fontSize: 12, color: 'var(--c-text-secondary)' }}>{formatDate(skill.createdAt)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span style={{ fontSize: 12, color: 'var(--c-text-muted)' }}>更新日期</span>
              <span style={{ fontSize: 12, color: 'var(--c-text-secondary)' }}>{formatDate(skill.updatedAt)}</span>
            </div>
            <div>
              <span style={{ fontSize: 12, color: 'var(--c-text-muted)', display: 'block', marginBottom: 4 }}>安装目录</span>
              <div
        ...
        {/* Compatible Apps */}
        <div className="glass-card p-4">
          <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--c-text)', margin: '0 0 12px' }}>兼容应用</h3>
          <div className="flex items-center gap-2 flex-wrap">
            {installLocations.map((loc) => {
              const info = toolIcons[loc.icon] || toolIcons[loc.toolName] || { color: '#6B7280', label: loc.toolName }
              const brandIcon = TOOL_ICONS[info.label] || TOOL_ICONS[loc.toolName]
              return (
                <div
                  key={loc.toolName}
  ...
  if (!content) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-3">
        ...
        <p style={{ color: 'var(--c-text-muted)', fontSize: 13 }}>暂无 README 内容</p>
      </div>
    )
  }
...
function LocationsTab({ locations, skillPath, skillId, onRefresh }: { locations: SkillDetailData['installLocations']; skillPath: string; skillId: string; onRefresh: () => void }) {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--c-text)', margin: 0 }}>安装位置管理</h3>
          <p style={{ fontSize: 11, color: 'var(--c-text-faint)', margin: '4px 0 0' }}>
            管理此技能在不同平台上的安装状态，点击路径可打开对应目录
          </p>
        </div>
        <span style={{ fontSize: 11, color: 'var(--c-accent)', background: 'rgba(99, 102, 241, 0.1)', padding: '3px 10px', borderRadius: 999, fontWeight: 600 }}>
          {installedCount}/{locations.length} 已安装
        </span>
      </div>
...
function VerifyTab({ contentHash, relatedFiles, installLocations, copiedHash, copyToClipboard }: {
  contentHash: string
  relatedFiles: string[]
  ...
  const installedLocations = installLocations.filter((l) => l.installed && l.hash)
  ...
  return (
    <div className="max-w-2xl space-y-4">
      ...
      <div className="glass-card p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--c-text)', margin: 0 }}>内容校验</h3>
        ...
        <div className="space-y-3">
          <div>
            <span style={{ fontSize: 11, color: 'var(--c-text-faint)', display: 'block', marginBottom: 4 }}>SKILL.md 内容哈希 (MD5)</span>
            <div
              className="flex items-center gap-2 px-3 py-2.5 rounded-lg"
              style={{ background: 'var(--c-bg-input)' }}
            >
              <Hash style={{ width: 13, height: 13, color: 'var(--c-accent)', flexShrink: 0 }} />
              <span style={{ fontSize: 12, color: 'var(--c-text-secondary)', fontFamily: 'monospace', letterSpacing: '0.02em' }} className="truncate flex-1">
                {contentHash || '无法计算'}
              </span>
              {contentHash && (
                <button
                ...
                >
                  {copiedHash ? <CheckCircle2 style={{ width: 14, height: 14, color: 'var(--c-green)' }} /> : <Copy style={{ width: 14, height: 14 }} />}
                </button>
              )}
            </div>
          </div>
          ...
          {installedLocations.length > 0 && (
            <div>
              <span style={{ fontSize: 11, color: 'var(--c-text-faint)', display: 'block', marginBottom: 6 }}>各安装位置哈希对比</span>
                  ...
                  return (
                    ...
                    >
                      ...
                      {hashMatch ? (
                        ...
                        <span style={{ fontSize: 10, color: '#DC2626', fontWeight: 600, flexShrink: 0 }}>不一致</span>
                      )}
                    </div>
                  )
                })}
      ...
      {/* File Integrity */}
      <div className="glass-card p-4">
        <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--c-text)', margin: '0 0 12px' }}>文件完整性</h3>
        <div className="space-y-2">
