...
export default function SidebarScanWidget() {
  ...
  const statusLabel = hasIssues ? t('sidebar.hasIssues', 'Issues') : updateSkills > 0 ? t('sidebar.updates', 'Updates') : t('sidebar.ok', 'OK')
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <strong style={{ fontSize: 12, color: 'var(--c-text)', fontWeight: 750 }}>{t('sidebar.scan', 'Scan')}</strong>
      ...
      <p style={{ marginTop: 9, fontSize: 10.5, color: 'var(--c-text-faint)' }}>
        {t('sidebar.skillCount', '{{count}} skills', { count: total || skills.length })}
        {errorSkills > 0 && <span style={{ color: 'var(--c-red)', marginLeft: 4 }}>· {errorSkills} {t('sidebar.errors', 'errors')}</span>}
        {updateSkills > 0 && <span style={{ color: 'var(--c-amber)', marginLeft: 4 }}>· {updateSkills} {t('sidebar.updates', 'updates')}</span>}
      </p>
      <p style={{ marginTop: 4, fontSize: 10.5, color: 'var(--c-text-faint)' }}>
        {t('sidebar.sourceCount', '{{count}} sources', { count: sources.length })} · {t('sidebar.toolCount', '{{count}} tools', { count: detectedTools })}
      </p>
    </div>
  )
}
